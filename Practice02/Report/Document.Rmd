...
2. Since the range now is smaller, the color will change better according to the value which allow us to have more information about our data (For example we can see how our data is distributed over the whole range).
3. With the new heatmap I can observe things I couldn't see before. For example I can find the low values much more easier.
...
Because before log2 the values where distributed between 0-> ~16000. this huge difference won't give a nice draw because it won't be sensitive to small or even large changes (by large I mean changes by hundreds.). For example if we took X30 (one of the noticable columns) we can notice that this column has higher values than X29 and X31.

```{r}
mean(data$X30-data$X29)
mean(data_log$X30-data_log$X29)
```
From the previous code we can see that log is more sensitive to changes than
...
```{r, eval = FALSE}
k <- 0 # Select more meaningful k by looking at Elbow plot 
...
```{r, eval = FALSE}
cluster_id <- 0 # choose a cluster you would like to analyse
...
library(dplyr)
head(arrange(..., ...)) # In console use ?function_name for help
```
...
pvalues <- apply(data_log, 1, function(x) t.test(x[...], x[...])$p.value)
...
Usually scientists are interested in significant p.values (<0.05), but in our case there are way too many significant proteins (can you identify how many?), so we will use only N the most significant proteins. 
...
```{r, eval = FALSE}
N <- 0 # choose number of proteins you want to select (e.g. 100)
...
modelFit # Check that model has been trained correctly
```
